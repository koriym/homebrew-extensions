class DdtraceAT81 < Formula
  ...
  def install
    php_config = "#{Formula["php@8.3"].opt_bin}/php-config"
  ...
  def safe_phpize
    ...
    system "#{Formula["php@8.3"].opt_bin}/phpize"
  end

  def php_version
    version = Utils.safe_popen_read("#{Formula["php@8.3"].opt_bin}/php", "-v")
    match = version.match(/PHP ([0-9]+\.[0-9]+)/)
    match ? match[1] : "8.3"
  end

  test do
    output = shell_output("#{Formula["php@8.3"].opt_bin}/php -m")
